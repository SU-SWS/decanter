...
module.exports = function () {
  return function ({ addComponents, theme }) {
    ...
    keys.forEach((key) => {
      if (largestScreen < parseInt(screens[key], 10))
        largestScreen = parseInt(screens[key], 10);

      if (theme(`decanter.screenMargins.${key}`)) {
        const gutterWidth = parseInt(
          theme(`decanter.screenMargins.${key}`),
          10
        );
    ...
    const key = `@media only screen and (min-width: ${
      largestScreen + largestGutter * 2
    }px)`;
    const components = {
